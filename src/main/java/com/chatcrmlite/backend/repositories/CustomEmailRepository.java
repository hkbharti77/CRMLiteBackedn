...
public interface CustomEmailRepository extends JpaRepository<CustomEmail, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT c FROM CustomEmail c WHERE c.tenant.id = :tenantId ORDER BY c.createdAt DESC")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(e.totalSent), 0) FROM CustomEmail e WHERE e.tenant.id = :tenantId AND e.createdAt >= :since")
