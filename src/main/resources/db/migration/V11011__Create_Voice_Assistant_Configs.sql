...
-- Note: table may already exist from an older Hibernate ddl-auto=update run; keep statements idempotent.
...
CREATE TABLE IF NOT EXISTS voice_assistant_configs (
    ...
    tts_voice_id VARCHAR(100) DEFAULT 'aura-asteria-en',
...
-- Align pre-existing Hibernate-created tables (no-op when CREATE above just ran)
ALTER TABLE voice_assistant_configs
    ALTER COLUMN id SET DEFAULT gen_random_uuid();

ALTER TABLE voice_assistant_configs
    ADD COLUMN IF NOT EXISTS tts_voice_id VARCHAR(100) DEFAULT 'aura-asteria-en';
...
-- Always supply id explicitly — never rely on column DEFAULT (may be missing on legacy tables)
INSERT INTO voice_assistant_configs (id, tenant_id, assistant_name, greeting_text, persona_prompt, enabled, version, created_at, updated_at)
SELECT gen_random_uuid(), id, 'Assistant', 'Hello! How can I help you today?', 'You are a helpful, professional AI voice assistant.', TRUE, 0, NOW(), NOW()
FROM tenants
